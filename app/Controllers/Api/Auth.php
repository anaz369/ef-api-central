...
class Auth extends BaseApiController
{
    ...
    public function token()
    {
        ...
        if (!$clientId || !$clientSecret) {
            $this->logRequest(null, null, '/api/auth/token', 'POST', $body, ['error' => 'missing_credentials'], 400, 0, 0);
        ...
        if (!$credential || !password_verify($clientSecret, $credential['client_secret_hash'])) {
            $this->logRequest(null, null, '/api/auth/token', 'POST', ['client_id' => $clientId], ['error' => 'invalid_credentials'], 401, 0, 0);
        ...
        $rawToken = $this->tokenModel->issueToken(
            (int) $credential['participant_id'],
            (int) $credential['id'],
            $environment,
            $ttl
        );
